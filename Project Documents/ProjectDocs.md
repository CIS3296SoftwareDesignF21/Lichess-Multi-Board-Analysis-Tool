...
## UML Diagram
The main page initializes the application by displaying the search panel and search functionality. It does so using the SearchHandler. By capturing user input from the search filter, the search handler is able to build and send a request to the Lichess API where we get all of our user game data. The Lichess API returns the search results to the handleResponse function, which calls parseResponse to obtain an array of games. The filterResultsByOpening uses the inputs that the user supplied to narrow down the search results, and the searchResults function presents a button to add the game to the GameBoardPanel for each individual result. The GameBoardPanel houses all the game board organization functions.
![UML](UML.png)

## Sequence Diagram 
The board management diagram contains the board addition and subtraction functionalities. The functions create or remove boardDivs which are added to or removed from the boardPanelDiv. When the "Add" button is clicked, the ModifyBoardPanel function passes the PGN from the row of the button to the BoardCreation function, which creates a new boardDiv element and adds it to a new boardPanelDiv. BoardCreation also calls the PGNV function, which injects the board into the boardDiv element. When the "X" button is clicked, ModifyBoardPanel sends the HTML id of the board to BoardRemoval, which calls the boardDivOrganize function and removes the corresponding board from its boardDiv.
